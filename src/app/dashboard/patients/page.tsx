...
export default function PatientsPage() {
  ...
  const patientsQuery = useMemoFirebase(() => {
    ...
    return query(
      ...
      where('finCode', '==', searchCriteria.finCode),
  ...
  return (
    <div className="space-y-8">
      ...
      {isLoading && <div className="flex justify-center items-center p-8"><Loader2 className="h-8 w-8 animate-spin text-primary"/> <span className="ml-4">Axtarılır...</span></div>}

      {error && (
          <Alert variant="destructive">
            <AlertTitle>Xəta</AlertTitle>
            <AlertDescription>Xəstə axtarışı zamanı xəta baş verdi. Zəhmət olmasa təhlükəsizlik qaydalarını yoxlayın.</AlertDescription>
          </Alert>
      )}
